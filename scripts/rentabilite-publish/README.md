...
**les mardi, jeudi et samedi** (3 fois/semaine), commit + push sur `main` (→ rebuild Vercel),
et envoie un **email récap** via Resend à chaque publication.

## Pourquoi progressif

Publier 898 pages d'un coup = signal spam pour Google. On lâche 3 villes 3×/semaine,
...
- `run.sh` — wrapper : filtre mardi/jeudi/samedi, pull, publie, commit/push, email. Pas de `claude -p` (zéro coût API).
